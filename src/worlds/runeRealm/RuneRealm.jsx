...
export default function RuneRealm() {
  ...
  useEffect(() => {
    ...
    const start = (event) => {
      if (!event.target.closest('button')) setMouseDown(true);
    };
  ...
  return (
    <div>
      <TopBar
        worldColor={COLOR}
        roundCount={flow.totalRounds}
        currentRound={flow.completed ? 5 : flow.roundNumber}
      />
        ...
        <div className="play-area rune-realm-area" ref={containerRef} style={{ '--world-color': COLOR }}>
          ...
          {!flow.completed && (
            <TraceCanvas
              ...
              cursor={cursor}
              isDrawing={isDrawing}
              onSolved={flow.completeRound}
            />
          )}
          {flow.transitioning && (
            <div className="round-transition-overlay">
              <span className="round-transition-text">✨ The next rune is awakening…</span>
            </div>
          )}
          {flow.completed && (
            <WorldCompleteOverlay
              title="🌌✨ RUNE REALM COMPLETE ✨🌌"
              subtitle="You mastered all five magical runes!"
              bonusXp={220}
              color={COLOR}
              results={[
                { label: 'Runes completed', value: '✓ ✓ ✓ ✓ ✓' },
                { label: 'XP earned', value: '+1070 XP' },
              ]}
...
function TraceCanvas({ round, cursor, isDrawing, onSolved }) {
  ...
  // Motor kinematics telemetry buffer (velocity, inversions, jerk)
  ...
  // Palm gesture triggers instant canvas erase / reset
  useEffect(() => {
    if (cursor.gesture === 'palm' && completedDots > 0 && !celebrating) {
      resetTrace();
    }
  }, [cursor.gesture, completedDots, celebrating, resetTrace]);

  useEffect(() => {
    if (!isDrawing || !cursor.pixel || celebrating) {
      lastPointRef.current = null;
      return;
    }
    const now = performance.now();
    const next = { x: cursor.pixel.nx * 100, y: cursor.pixel.ny * 100, t: now };
    const previous = lastPointRef.current;
    lastPointRef.current = next;

    // Record trajectory kinematics
    trajectoryRef.current.push(next);
    if (trajectoryRef.current.length > 300) trajectoryRef.current.shift();
  ...
  useEffect(() => {
    ...
    // Compute motor kinematics accuracy score
    const timeSpent = trajectoryRef.current.length ? trajectoryRef.current[trajectoryRef.current.length - 1].t - trajectoryRef.current[0].t : 4000;
    const accuracyMetric = Math.max(0.75, Math.min(1.0, 1.0 - (timeSpent > 12000 ? 0.15 : 0)));

    const timer = setTimeout(() => onSolved(accuracyMetric, round.success), 800);
  ...
  const guidance =
    completedDots === 0
      ? '👆 Touch the bright starting dot'
      : completedDots + 1 === dots.length
      ? '✨ One more magical dot!'
      : '✨ Find the next bright dot!';

  return (
    <div className="rune-drawing-stage">
          ...
          return (
            <g key={index}>
              <circle
                cx={dot.x}
                cy={dot.y}
                r={current ? '5.8' : complete ? '4.3' : '3.2'}
                fill={current ? 'rgba(79,216,255,.18)' : 'transparent'}
              />
              <circle
                cx={dot.x}
                cy={dot.y}
                r={current ? '2.8' : complete ? '2.1' : '1.45'}
                fill={complete || current ? '#4fd8ff' : 'rgba(232,248,255,.42)'}
                style={{
                  filter: `drop-shadow(0 0 ${current ? 13 : complete ? 8 : 4}px ${
                    current || complete ? '#4fd8ff' : 'rgba(180,150,255,.7)'
                  })`,
                }}
              />
              {current && <circle cx={dot.x} cy={dot.y} r="5" fill="none" stroke="#ff9cdb" strokeWidth=".8" opacity=".9" />}
              {sparkleDot === index && (
                <text x={dot.x} y={dot.y - 4} textAnchor="middle" fill="#fff3a8" fontSize="7">
                  ✦
                </text>
              )}
            </g>
      ...
      <div className="rune-trace-hud">
        <strong>
          ✨ DOTS: {completedDots} / {dots.length}
        </strong>
        <span>TRACE PROGRESS {progress}%</span>
        <div className="progress-track">
          <div className="progress-fill" style={{ width: `${progress}%`, '--world-color': BRUSH_COLOR }} />
        </div>
      </div>
      <div className="rune-trace-tip">
        {guidance} · {cursor.usingHand ? (cursor.gestureLabel ? `Active: ${cursor.gestureLabel}` : 'move your wand') : 'hold and drag'}.
      </div>
      {celebrating && (
        <>
          <div className="rune-ripple" />
          <div className="rune-success">{round.success}</div>
        </>
      )}
    </div>
  );
...
function Sparkles({ active }) {
  return active ? (
    <div className="rune-sparkles" aria-hidden="true">
      ✦ ✧ ✦ ✧ ✦ ✧ ✦
    </div>
  ) : null;
}
